...
class PolyGNN(nn.Module):
    def __init__(self,
        ...
        self.first_annotation = FirstAnnotation(28, 128, 1)
    ...
    def forward(self, x, init_polys):
        ...
        psp_out1 = conv_layers[0]
        #psp_out = psp_out1+psp_out  #modify by dzh 7.23
        #print(psp_out.shape)
        edge_logits, vertex_logits, logprob, _ = self.first_annotation.forward(psp_out1)
        #print(psp_out.shape)
        out_dict['edge_logits'] = edge_logits
        out_dict['vertex_logits'] = edge_logits#vertex_logits
        del(conv_layers)
        ...
        feature_with_edges = torch.cat([psp_out, edge_logits], 1)
        #print(feature_with_edges.shape)
        out_dict['feature_with_edges'] = feature_with_edges
        out_dict['feature_edges'] = edge_logits
        conv_layers = [self.encoder.edge_annotation_cnn(feature_with_edges)]
        #conv_layers = feature_with_edges
        out_dict['pred_polys'] = []

        for i in range(self.coarse_to_fine_steps):
            
            if i == 0:
                component = utils.prepare_gcn_component(init_polys.cpu().numpy(),
                                                        self.psp_feature,
                                                        init_polys.size()[1],
                                                        n_adj=self.n_adj)
