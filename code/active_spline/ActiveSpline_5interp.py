...
class ActiveSplineTorch():
    ...
    def sample_point(self, cps):
        ...
        auxillary_cps = torch.zeros(cps.size(0), cps.size(1) + 4, cps.size(2)).to(self.device)
        auxillary_cps[:, 2:-2, :] = cps
        ...
        l_02 = torch.sqrt(torch.sum(torch.pow(cps[:, 1, :] - cps[:, 2, :], 2), dim=1) + EPS)
        l_last_02 = torch.sqrt(torch.sum(torch.pow(cps[:, -2, :] - cps[:, -3, :], 2), dim=1) + EPS)

        l_01.detach_().unsqueeze_(1)
        l_last_01.detach_().unsqueeze_(1)
        l_02.detach_().unsqueeze_(1)
        l_last_02.detach_().unsqueeze_(1)

        # print(l_last_01, l_01)

        auxillary_cps[:, 0, :] = cps[:, 0, :] - l_01 / l_last_01 * (cps[:, -1, :] - cps[:, -2, :])
        
        auxillary_cps[:, 1, :] = cps[:, 1, :] - l_02 / l_last_02 * (cps[:, -2, :] - cps[:, -3, :])
        auxillary_cps[:, -2, :] = cps[:, -2, :] - l_last_02 / l_02 * (cps[:, 2, :] - cps[:, 1, :])

        auxillary_cps[:, -1, :] = cps[:, -1, :] + l_last_01 / l_01 * (cps[:, 1, :] - cps[:, 0, :])
        # auxillary_cps[:, -2, :] = cps[:, -2, :] + l_last_01 / l_01 * (cps[:, 1, :] - cps[:, 0, :])
        # print(auxillary_cps.shape)
        ...
        for sg in range(2, self.cp_num+2):
            v = self.batch_linspace(t[:, sg], t[:, sg+2], self.p_num)
            # print(v.type())
            # print(v.size())
            # print(v)
            t0 = t[:, sg-2].unsqueeze(1)
            t1 = t[:, sg-1].unsqueeze(1)
            t2 = t[:, sg].unsqueeze(1)

            t3 = t[:, sg+1].unsqueeze(1)
            t4 = t[:, sg+2].unsqueeze(1)
            t5 = t[:, sg+3].unsqueeze(1)

            for i in range(self.p_num/2):

                tv = v[:, i].unsqueeze(1)

                x01 = (t1-tv)/(t1-t0)*auxillary_cps[:, sg-2, :]+(tv-t0)/(t1-t0)*auxillary_cps[:, sg-1, :]

                x12 = (t2-tv)/(t2-t1)*auxillary_cps[:, sg-1, :]+(tv-t1)/(t2-t1)*auxillary_cps[:, sg,:]

                x23 = (t3-tv)/(t3-t2)*auxillary_cps[:, sg, :]+(tv-t2)/(t3-t2)*auxillary_cps[:, sg+1, :]

                x34 = (t4-tv)/(t4-t3)*auxillary_cps[:, sg+1, :]+(tv-t3)/(t4-t3)*auxillary_cps[:, sg+2, :]

                x45 = (t5-tv)/(t5-t4)*auxillary_cps[:, sg+2, :]+(tv-t4)/(t5-t4)*auxillary_cps[:, sg+3, :]

                x012 = (t2-tv)/(t2-t0)*x01+(tv-t0)/(t2-t0)*x12

                x123 = (t3-tv)/(t3-t1)*x12+(tv-t1)/(t3-t1)*x23

                x234 = (t4-tv)/(t4-t2)*x23+(tv-t2)/(t4-t2)*x34

                x345 = (t5-tv)/(t5-t3)*x34+(tv-t3)/(t5-t3)*x45

                points[:, lp] = (t2-tv)/(t2-t1)*x012+(tv-t1)/(t2-t1)*x123 
                points[:, lp + 1] = (t4-tv)/(t4-t3)*x234 + (tv-t3)/(t4-t3)*x345
